...
@section('content')
    @include('backend.common.common-header', ['heading' => 'Size Managment'])
        ...
        <div class="card">
            ...
            <div class="card-body">
                @if ($view_type == 'ADD' || $view_type == 'EDIT')
                    <form id="sizeForm"
                        action="{{ $view_type == 'ADD' ? route('backend.size.store') : route('backend.size.update', $size->id) }}"
                        method="POST">
                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-md-5">
                                    <input type="text" class="form-control" name="code" id="code"
                                        @isset($size->code) value="{{ $size->code }}" @endisset>
                                </div>
                            </div>
                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-md-5">
                                    <input type="number" class="form-control" name="size" id="size"
                                        @isset($size->size) value="{{ $size->size }}" @endisset>
                                </div>
                            </div>
                                    ...
                                    @foreach ($status as $key => $value)
                                        <div class="form-check form-check-inline">
                                            ...
                                            <input class="form-check-input" type="radio" name="status" id="inlineRadio1"
                                                value="{{ $key }}"
                                                @isset($size->status) @checked($size->status == $key)@endisset>
